...
def compute(program, istream: queue.Queue, ostream: queue.Queue, memsize):
    opcodes = {
        1: add,
        2: multiply,
        3: geninput(istream),
    ...
    memory = list(map(int, program.split(","))) + [0 for _ in range(memsize)]
...
class Computer:
    def __init__(self, memsize=10_000):
    ...
    def run(self, program):
        self.process = threading.Thread(target=compute, args=(program, self.istream, self.ostream, self.memsize))
        self.process.start()
